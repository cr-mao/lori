...
func (c *Client) Restart() {
	...
	go func() {
		...
		case "websocket":
			...
			if err != nil {
				// Conection failed
				log.Errorf("WsClient connect to server failed, err:%v", err)
				c.ErrChan <- err
				return
			}
		...
		default:
			...
			if c.useTLS {
				...
				if err != nil {
					log.Errorf("tls client connect to server failed, err:%v", err)
					c.ErrChan <- err
					return
				}
			} else {
				...
				if err != nil {
					// Conection failed
					log.Errorf("client connect to server failed, err:%v", err)
					c.ErrChan <- err
					return
				}
			}
		...
		log.Infof("[START] Zinx Client LocalAddr: %s, RemoteAddr: %s\n", c.conn.LocalAddr(), c.conn.RemoteAddr())
		// HeartBeat detection
		if c.hc != nil {
...
func (c *Client) Stop() {
	log.Infof("[STOP] Zinx Client LocalAddr: %s, RemoteAddr: %s\n", c.conn.LocalAddr(), c.conn.RemoteAddr())
	c.conn.Stop()
	c.exitChan <- struct{}{}
	close(c.exitChan)
	close(c.ErrChan)

...
func (mh *MsgHandle) GetTaskQueueWorkerId(request iface.IRequest) uint64 {
...
func (mh *MsgHandle) SendMsgToTaskQueue(request iface.IRequest) {
	...
	// log.Debugf("Add ConnID=%d request msgID=%d to workerID=%d", request.GetConnection().GetConnID(), request.GetMsgID(), workerID)
	// Send the request message to the task queue
	mh.TaskQueue[workerID] <- request
	log.Debugf("SendMsgToTaskQueue-->%s", hex.EncodeToString(request.GetData()))
}

// doFuncHandler handles functional requests (执行函数式请求)
func (mh *MsgHandle) doFuncHandler(request iface.IFuncRequest, workerId int) {
	defer func() {
		if err := recover(); err != nil {
			log.Errorf("doFuncRequest panic: %v", err)
		}
	}()
...
func (mh *MsgHandle) doMsgHandler(request iface.IRequest, workerID int) {
	defer func() {
		if err := recover(); err != nil {
			log.Errorf("doMsgHandler panic: %v", err)
		}
	}()
	...
	if !ok {
		log.Errorf("api msgID = %d is not FOUND!", request.GetMsgID())
		return
	}
